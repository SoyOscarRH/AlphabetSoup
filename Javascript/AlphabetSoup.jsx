...
export default class AlphabetSoup extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    if (
      props.HorizontalSize == state.HorizontalSize &&
      props.Category === state.Category &&
      props.VerticalSize == state.VerticalSize &&
      !state.FirstTime
    )
      return null;
    ...
    while (
      ...
      for (let i = 0; i < 3; i++) {
        // Horizontal
        if (Style === 0) {
          const Possible = Words[WordIndex].split("").every(function(
            element,
            index
          ) {
            if (index + StartHorizontalIndex >= HorizontalSize) return false;
            else if (
              Board[StartVerticalIndex][StartHorizontalIndex + index].Letter !==
                Words[WordIndex].charAt(index) &&
              Board[StartVerticalIndex][StartHorizontalIndex + index].Letter !==
                "0"
            )
              return false;
          ...
          if (Possible) {
            Words[WordIndex].split("").forEach(function(element, index) {
              Board[StartVerticalIndex][
                StartHorizontalIndex + index
              ].Letter = element;
              Board[StartVerticalIndex][
                StartHorizontalIndex + index
              ].IsWord = true;
              Board[StartVerticalIndex][
                StartHorizontalIndex + index
              ].WordNumber.push(WordAddedIndex);
              Board[StartVerticalIndex][
                StartHorizontalIndex + index
            ...
            WordsInfo.push({
              ...
              Found: Array(Words[WordIndex].length).fill(false)
            });
            WordAddedIndex += 1;
            break;
          }
        ...
        // Vertical
          ...
          const Possible = Words[WordIndex].split("").every(function(
            ...
            if (index + StartVerticalIndex >= VerticalSize) return false;
            else if (
              Board[StartVerticalIndex + index][StartHorizontalIndex].Letter !==
                Words[WordIndex].charAt(index) &&
              Board[StartVerticalIndex + index][StartHorizontalIndex].Letter !==
                "0"
            )
          ...
          if (Possible) {
            Words[WordIndex].split("").forEach(function(element, index) {
              Board[StartVerticalIndex + index][
                StartHorizontalIndex
              ].Letter = element;
              Board[StartVerticalIndex + index][
                StartHorizontalIndex
              ].IsWord = true;
        ...
        // Diagonal
          ...
          let Possible = false;
          try {
            Possible = Words[WordIndex].split("").every(function(
              element,
              index
            ) {
              if (index + StartHorizontalIndex >= HorizontalSize) return false;
              else if (
                Board[StartVerticalIndex + index][StartHorizontalIndex + index]
                  .Letter !== Words[WordIndex].charAt(index) &&
                Board[StartVerticalIndex + index][StartHorizontalIndex + index]
                  .Letter !== "0"
              )
                return false;
    ...
    const Press = (i, j, Element) => {
      ...
      if (Element.IsWord) {
        ...
        Element.WordNumber.forEach((actualNumber, index) => {
          NewWordsInfo[actualNumber].Found[
            Element.WordIndex[index]
          ] = !NewWordsInfo[actualNumber].Found[Element.WordIndex[index]];
        ...
        if (CompleteWord) {
          M.toast({
            html: `Nueva palabra encontrada: ${this.state.WordsInfo[
              CompleteWordIndex
            ].Word.toUpperCase()}`
          });
        }
    ...
    const BoardRepresentation = this.state.Board.map((Row, i) => {
      const RowRepresentation = Row.map((Element, j) => {
        ...
        if (Element.IsWord) {
          ...
          Element.WordNumber.forEach((actualNumber, index) => {
            if (
              this.state.WordsInfo[actualNumber].Found.every(element => element)
            )
              CompleteWord = true;
          });
        }
      ...
      <div>
            ...
            <div className="col s10 offset-s1 blue-grey-text darken-2-text">
              ...
              >
                <table className="browser-default">
                  <tbody>
                    <tr>
                      {this.state.WordsInfo.filter(
                        Element => !Element.Found.every(e => e)
                      ).map(Element => {
                        return (
                          <td key={Element.Word}>
                            <div
                            ...
                            >
                              <img
                                src={`Distribution/${this.props.Category}/${
                                  Element.Word
                                }${
                                  this.props.Category === "Paises"
                                    ? ".jpg"
                                    : ".jpeg"
                                }`}
                                style={{ width: "80%" }}
                              />
                              <span>
                                <strong>{Element.Word.toUpperCase()}</strong>
                              </span>
                            </div>
                          </td>
                        );
        ...
        <div className="card-panel z-depth-2">
          ...
          <div id="CardTable" style={{ overflowX: "scroll" }}>
            <table className="browser-default">
              <tbody>{BoardRepresentation}</tbody>
            </table>
          </div>
        </div>
      </div>
